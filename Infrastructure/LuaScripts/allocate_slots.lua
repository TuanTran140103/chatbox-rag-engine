...
-- Field: {documentId} -> Value: { "allowSlot": int, "used": int, "remainingWork": int, "totalWork": int }
...
-- ARGV[3]: workerDataJson  { allowSlot:0, used:0, remainingWork:N, totalWork:N }
--
-- Gọi 2 lần per job:
--   Phase 1 start: remainingWork = 2            (1 GenSummary + 1 GenQASummary)
--   Phase 2 start: remainingWork = chunks.Count (overwrite, Lua tái phân bổ ngay)
...
-- 1. Lưu cấu hình hệ thống vào Hash (field đặc biệt __config_total_max)
redis.call('HSET', model_key, '__config_total_max', total_max)

-- 2. Cập nhật/Thêm job hiện tại (HSET ghi đè nếu đã tồn tại → dùng được cho phase 2)
...
for i = 1, #all_raw, 2 do
    ...
    -- Bỏ qua field cấu hình, chỉ lấy job data
...
-- Bước A: Chia đều (tối đa remainingWork của từng job)
for _, w in ipairs(workers) do
    local take = math.min(w.remainingWork, base_share)
    w.allowSlot = take
    remaining_slots = remaining_slots - take
end

-- Bước B: Chia dư — ưu tiên job có nhiều công việc còn lại hơn
while remaining_slots > 0 do
    table.sort(workers, function(a, b)
        local a_gap = a.remainingWork - a.allowSlot
        local b_gap = b.remainingWork - b.allowSlot
        if a_gap == b_gap then return a.remainingWork > b.remainingWork end
        return a_gap > b_gap
    end)

    local distributed = 0
    for _, w in ipairs(workers) do
        if remaining_slots > 0 and w.allowSlot < w.remainingWork then
