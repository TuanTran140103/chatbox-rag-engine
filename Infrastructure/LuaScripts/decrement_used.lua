...
--
-- Sau khi 1 LLM call hoàn thành:
--   used         -= 1  (đã trả slot)
--   remainingWork -= 1  (xong 1 công việc)
-- Nếu remainingWork < allowSlot → tái phân bổ slot cho tất cả job

local model_key = KEYS[1]
local worker_id = ARGV[1]

-- 1. Đọc total_max từ Hash
...
if current_data.remainingWork > 0 then current_data.remainingWork = current_data.remainingWork - 1 end

-- Nếu remainingWork vẫn >= allowSlot thì không cần tái phân bổ (slot vẫn còn dùng được hết)
local needs_redistribution = current_data.remainingWork < current_data.allowSlot
...
-- 4. Tái phân bổ: job sắp xong → nhường slot cho job khác còn nhiều việc hơn
